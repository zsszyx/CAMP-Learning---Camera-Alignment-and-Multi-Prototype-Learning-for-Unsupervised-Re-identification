...
def extract_imgs(data_loader, print_freq=50):
...
def pairwise_distance(features, query=None, gallery=None):
    ...
    q_f = [f for f, _, _ in query]
    g_f = [f for f, _, _ in gallery]
    torch.save(dist_m, 'qf_dist.pt')
    torch.save(q_f, 'qf.pt')
    torch.save(g_f, 'gf.pt')
